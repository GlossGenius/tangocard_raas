...
module TangocardRaas
  class OrderRewardView
    # Verbose list of credentials
    attr_accessor :credential_list

    # Credentials as a map with String label key and credential value String
    ...
    def self.openapi_types
      {
        :'credential_list' => :'Array<CredentialView>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TangocardRaas::OrderRewardView` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TangocardRaas::OrderRewardView`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [credential_list, credentials, redemption_instructions].hash
    end
